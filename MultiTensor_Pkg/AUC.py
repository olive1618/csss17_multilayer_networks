...
def calculate_AUC_weighted(M,max_w):
    """"
    AUC for wieghted entries A_ij. Ideally, if an observed edge has A_ij=4, it will have a bigger M entry than an observed edge with A_ij<4
    - M= # SORTED (from small to big) List of 2-tuple, each entry is M[n]=(mu_ij,A_ij)    
    ...
    pos=0;neg=0
    y=np.zeros(max_w+1);bad=0.; 
    for m,a in M:
        y[a]+=1;   # number of entries with A_ij=a already encountered
        for q in range(a+1,max_w+1):bad+=y[q]   # number of entries with A_ij>a already encountered  --> penalty
    # calculate denominator --> normalization    
    Z=0;
    for k in range(max_w+1):
        m=0.
        for q in range(k+1,max_w+1): m+=y[q]
        Z+=m*y[k]
